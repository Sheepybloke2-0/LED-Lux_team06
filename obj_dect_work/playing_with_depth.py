...
# ------------- Functions -----------------------------------
def calc_distance(knownWidth, focalLength, perWidth):
...
if __name__ == '__main__':
    ...
    # Start the Webcam
    webcamA = WebcamVideoStream(src=0).start()
    webcamB = WebcamVideoStream(src=1).start()

    stereo = cv2.StereoSGBM_create(numDisparities=0, blockSize=3, uniquenessRatio=5)

    first_frame = True
    # Start the tensorflow session
    while(1):
        no_err, imgA = webcamA.getFrame()
        no_err, imgB = webcamB.getFrame()
        if no_err == True:
            input_frame_q.put(imgA)
            ...
            for idx in person_idx:
                temp_box = []
                temp_box.append(boxes[0][idx][0] * HEIGHT)
                temp_box.append(boxes[0][idx][1] * WIDTH)
                temp_box.append(boxes[0][idx][2] * HEIGHT)
                temp_box.append(boxes[0][idx][3] * WIDTH)

                # Draw point halfway down the image in the center of the box
                mid_y = temp_box[0] + ((temp_box[2] - temp_box[0])/2)
                mid_x = temp_box[1] + ((temp_box[3] - temp_box[1])/2)
                print(mid_x, mid_y)

                # Draw the mid point
                cv2.circle(img_brg, (int(mid_x), int(mid_y)), 10, (0,255,120), -1)

            # Need this for the disparity
            imgA_gray = cv2.cvtColor(imgA, cv2.COLOR_BGR2GRAY)
            imgB_gray = cv2.cvtColor(imgB, cv2.COLOR_BGR2GRAY)

            disparity = stereo.compute(imgA_gray, imgB_gray)
            cv2.imshow("img", img_brg)
            cv2.imshow("img2", disparity)
        else:
