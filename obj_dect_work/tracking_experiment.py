...
def worker_node(input_frame_q, output_dict_q):
    ...
    while True:
        ...
        output_dict = detect_objects(frame_rgb, detection_graph, sess)
...
if __name__ == '__main__':
    ...
    pool = Pool(1 , worker_node, (input_frame_q, output_dict_q))
    ...
    while(1):
        no_err, img = webcam.getFrame()
        if no_err == True:
            # Put in the queue every 10 frames
            if frame_count is 0 and first_frame is True:
                ...
                for id in classes_index:
                    ...
                    if scores[0][current_index] > 0.50:
                        ...
                        found_obj.append(id)
                        print("Object %s with score %s at position %s" % (id, scores[0][current_index], boxes[0][current_index]) )
                        if id == 1:
                            person_idx = current_index

            # If a person was found, trackem!
            if first_frame == True:
                # Normazile the image to work with the scale. Tensorflow coords are (ymin, xmin, ymax, xmax)
                temp_box = []
                temp_box.append(boxes[0][person_idx][0] * HEIGHT)
                temp_box.append(boxes[0][person_idx][1]* HEIGHT)
                temp_box.append(boxes[0][person_idx][2] * WIDTH)
                temp_box.append(boxes[0][person_idx][3] * WIDTH)
                bbox = tuple(temp_box)
                print(bbox, person_idx)
